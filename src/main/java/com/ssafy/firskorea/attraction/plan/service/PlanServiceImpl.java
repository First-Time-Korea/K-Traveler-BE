package com.ssafy.firskorea.attraction.plan.service;
...
import com.ssafy.firskorea.attraction.plan.mapper.PlanMapper;
...
public class PlanServiceImpl implements PlanService {
    ...
    public PlanServiceImpl(PlanMapper planMapper, ObjectMapper objectMapper) {
    ...
    /**
     * DB 프로시저를 이용해 여행 계획 관련하여 모든 삽입을 한다.
     *
     * @param dto 여행 계획 생성 요청 데이터 전송 객체
     */
    @Override
    ...
    private Map<String, Object> createParamsMap(PlanCreationDto dto) {
        Map<String, Object> params = new HashMap<>();
        params.put("memberId", dto.getMemberId());
        params.put("title", dto.getTitle());
        addThumbnailParams(dto.getPlanThumbnailDto(), params);
        return params;
    }

    private void addThumbnailParams(PlanThumbnailDto thumbnail, Map<String, Object> params) {
        if (thumbnail != null) {
            params.put("saveFolder", thumbnail.getSaveFolder());
            params.put("originFile", thumbnail.getOriginFile());
            params.put("saveFile", thumbnail.getSaveFile());
        } else {
            params.put("saveFolder", null);
            params.put("originFile", null);
            params.put("saveFile", null);
        }
    }

    /**
     * 프로시저에서 사용하기 위해 Json으로 변경한다.
     */
    private String convertAttractionsToJson(PlanCreationDto dto) throws JsonProcessingException {
        return objectMapper.writeValueAsString(dto.getPlanDateAttractionDtos());
    }
}
